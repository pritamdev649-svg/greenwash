...
const ClothTypes: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', wash_price: 0, iron_price: 0, dry_clean_price: 0 });
  const [editingId, setEditingId] = useState<string | null>(null);

  const fetchItems = async () => {
    try {
      setLoading(true);
      const data = await orderService.getAllClothTypes();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', wash_price: 0, iron_price: 0, dry_clean_price: 0 });
      setEditingId(null);
      fetchItems();
    } catch (err: any) {
      console.error(err);
      ...
      <div className="grid lg:grid-cols-5 gap-12">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="pt-4 flex gap-3">
                  {editingId && (
                    <button 
                      type="button" 
                      onClick={() => { setEditingId(null); setFormData({ name: '', wash_price: 0, iron_price: 0, dry_clean_price: 0 }); }} 
        ...
        <div className="lg:col-span-3 space-y-6">
           <div className="flex items-center gap-2 mb-4">
              <h3 className="font-bold text-slate-800">Current Catalog</h3>
           ...
           <div className="grid gap-4">
              {loading ? (
                ...
                <div className="p-20 text-center card bg-slate-50/50 border-dashed border-2">
                   <p className="text-slate-400 font-medium">No items in catalog. Start by adding your first service type.</p>
                </div>
              ) : items.map((item) => (
